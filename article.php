...
<div class="container">
    ...
    <div class="article-page-header">
        <h1 class="article-page-title"><?= htmlspecialchars($article['name']); ?></h1>
        <h2 class="article-page-subtitle"><?= $article['header']; ?></h2>
        <p class="article-page-author">By <?= htmlspecialchars($article['author']); ?></p>
        <?php
        $date = new DateTime($article['date']);
        $formattedDate = strtoupper($date->format('F j, Y'));
        ?>
        <p class="article-page-date">Published <?= htmlspecialchars($formattedDate); ?></p>
        <p class="article-page-reading-time"><?= htmlspecialchars($article['duree_reading']); ?> min read</p>
    </div>
    <hr class="horizontal-line">
    <figure class="article-page-image-container">
...
.003z"/>
    ...
    <script>
        ...
        function shareOnTwitter() {
            const url = getCleanUrl();
            const text = document.querySelector('.article-page-title').innerText;
        ...
        function shareViaEmail() {
            const url = getCleanUrl();
            const subject = document.querySelector('.article-page-title').innerText;
    ...
    <?php if (!empty($related_articles)): ?>
        <div class="related-articles-container">
            ...
            <di v class="related-articles-row mt-4">
                <?php foreach ($related_articles as $related_article): ?>
                    <div class="related-articles-col mb-4">
                        <a href="article.php?id=<?= htmlspecialchars($related_article['id']); ?>"
                           class="related-articles-link">
                            <div class="related-articles-card">
                                <img class="related-articles-img-top"
                                     src="<?= htmlspecialchars($related_article['image']); ?>"
                                     alt="Article Image">
                                <div class="related-articles-card-body">
                                    <h5 class="related-articles-title"><?= htmlspecialchars($related_article['name']); ?></h5>
                                </div>
                            </div>
                        </a>
                    </div>
